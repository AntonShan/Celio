...
const uint8ToInt16 = utils_1.convert('Uint8')('Int16');
const uint8ToUint16 = utils_1.convert('Uint8')('Uint16');
const uint8ToUint32 = utils_1.convert('Uint8')('Uint32');
const uint8ToFloat32 = utils_1.convert('Uint8')('Float32');
const HEADER_OFFSET = 14;
class DATReader {
    constructor(data) {
        const header = [...data].slice(0, FILE_HEADER.length)
            .map(utils_1.Utils.charOf)
            .join('');
        const version = uint8ToUint16(data.slice(FILE_HEADER.length, FILE_HEADER.length + 2));
        if (header !== FILE_HEADER) {
            ...
            this.data = [...data];
            this.starsInFile = uint8ToUint32(data.slice(FILE_HEADER.length + 2, FILE_HEADER.length + 6));
        }
    }
    read() {
        let result = [];
        ...
        while (starNumber < this.starsInFile) {
            ...
            let SpectralType = utils_1.decodeSpectralClass(uint8ToUint16(this.data.slice(offset + 18, offset + 20)));
            result.push({
                isStar: true,
